...
const getClientData = (dir, config) => {
  return co(function* () {
    ...
    const loginData = (configFile) ? yield fsUtils.getFileContent(path.join(dir, loginFile)) : '';
...
const createAuthClient = (client, authClientInfo) => {
  return co(function* () {
    ...
    if (existingAuthClient) {
      ...
      const delta = (existingAuthClient.global) ? _.merge({}, authClientInfo.config):
        _.merge({}, baseClientConfig, authClientInfo.config);
