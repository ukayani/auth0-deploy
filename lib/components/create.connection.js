...
const create = (client, dir, config) => {
  return co(function* () {
    ...
    const connections = yield fsUtils.getDirectories(dir);

    for (let i in connections) {
      const connectionData = yield getConnectionData(connections[i], config);

      yield createConnection(client, connectionData);
    }
  })();
};
...
const getConnectionData = (dir, config) => {
  return co(function* () {
    ...
    for (let i in scripts) {
      ...
      scriptData[scriptKey] = yield fsUtils.getFileContent(path.join(dir, script));
    }
...
const createConnection = (client, connectionInfo) => {
  return co(function* () {
    ...
    if (existingConnection) {
      ...
      console.log('Config', delta.options.bareConfiguration);
